...
test.describe("Directional view transitions", () => {
  ...
  test("untyped navigation carries no direction", async ({ page }) => {
    // The header tabs are chrome rendered at every depth, so they are
    // deliberately untyped — a "forward" slide would point the wrong way
    // whenever the reader clicks Blog from a post.
    await page.goto("/en")
    const supported = await page.evaluate(
      () => typeof document.startViewTransition === "function",
    )
    ...
    await Promise.all([
      page.waitForURL(/\/en\/blog$/),
      clickHydrated(page, "header a[href='/en/blog']"),
    ])
