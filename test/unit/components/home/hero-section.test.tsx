...
describe("HeroSection", () => {
  ...
  // The HUD strip under the buttons (post count, tag count, latest date) is
  // gone; the numbers live on the blog index now, beside the sort control.
  test("carries no metrics strip", () => {
    const dictionary = getDictionary("en")
    const { container } = render(
      <HeroSection locale="en" dictionary={dictionary} />,
    )
