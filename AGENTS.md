...
- Two preloads run before every test, both in `test/unit/` and registered in
  `bunfig.toml`: `happydom.ts` for the DOM environment, and
  `setup-next-navigation-mock.ts` for the App Router hooks. They are global —
  do not re-register them per file
