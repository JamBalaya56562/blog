...
<CommitGraph
  ...
  hint="Step forward and the jj log above moves with you. Rows that changed are marked."
  scenes={[
    {
      command: "jj log",
      caption: "Just after changing Hello to Hello!, before the squash",
      rows: [
        { changeId: "ovvvnnor", desc: "Add a licence note", kind: "at", note: "LICENSE, and the edited README.md" },
        { changeId: "oklvxylu", desc: "Greet the world", commitId: "bf873b9b", kind: "commit" },
        { changeId: "tturtmot", desc: "Add README", commitId: "4bb47a91", kind: "commit" },
        { changeId: "zzzzzzzz", desc: "root()", commitId: "00000000", kind: "immutable" },
      ],
    },
    {
      command: "jj squash README.md",
      caption: "Only README.md moved to the parent; oklvxylu keeps its change ID and takes a new commit ID",
      rows: [
        { changeId: "ovvvnnor", desc: "Add a licence note", commitId: "5e6ce646", kind: "at", rewritten: true, note: "LICENSE stayed; the change was rebased for you" },
  ...
  caption="The change ID stays and only the commit ID is new; whatever sat on top is rebased for you"
/>
...
<CommitGraph
  ...
  hint="Step forward and @ drops two rows, taking the two above it along."
  scenes={[
    {
      command: "jj log",
      caption: "Where the squash left things: @ is at the tip",
      rows: [
        { changeId: "ovvvnnor", desc: "Add a licence note", kind: "at" },
        { changeId: "oklvxylu", desc: "Greet the world", kind: "commit" },
        { changeId: "tturtmot", desc: "Add README", commitId: "4bb47a91", kind: "commit" },
        { changeId: "zzzzzzzz", desc: "root()", commitId: "00000000", kind: "immutable" },
      ],
    },
    {
      command: "jj edit tturtmot && echo node_modules/ > .gitignore",
      caption: "Moved to tturtmot and put .gitignore there; its two descendants were rebased",
      rows: [
        { changeId: "ovvvnnor", desc: "Add a licence note", kind: "commit", rewritten: true, note: "Rebased 2 descendant commits" },
        { changeId: "oklvxylu", desc: "Greet the world", kind: "commit", rewritten: true },
        { changeId: "tturtmot", desc: "Add README", commitId: "2610b68f", kind: "at", note: ".gitignore and README.md" },
        { changeId: "zzzzzzzz", desc: "root()", commitId: "00000000", kind: "immutable" },
      ],
    },
  ]}
  ...
  status="{n}/{total} · {command} — {caption}"
  caption="Move to an earlier change and fix it there; jj rebases whatever sat on top"
/>
...
Here are the same three commands, run against Git and against jj side by side.

<BookmarkGraph
  title="git / jj"
  hint="Press a command and both graphs move. Watch which main comes along."
  ...
  commands={{
    ...
    bookmark: { git: "(moves by itself)", jj: "jj bookmark set main -r @-" },
  ...
  status={{
    initial: "One Add README on each side; in jj the empty @ sits above it",
    commit: "Committed {desc}. Git's main came along; jj's stayed where it was put",
    bookmark: "Moved jj's main up to the commit just written. Git has no such step",
    push: "What the remote holds is immutable (◆) now. The @ is still yours to rewrite",
    pushGitOnly: "Git sent it. jj found nothing to send: the bookmark is still behind",
  }}
  caption="A Git branch follows the commits. A jj bookmark stays where you put it, and moving it is your step"
/>
...
<CommitGraph
  ...
  hint="The first step is just after the fetch: @ is still on the old main, and only the rebase moves it."
  scenes={[
    {
      command: "jj git fetch",
      caption: "main moved on; your change is still on the old one",
      rows: [
        { changeId: "upwouywx", desc: "Add contributing guide", commitId: "0be7156b", kind: "at" },
        { changeId: "zumzptvl", desc: "Add the copyright line", commitId: "eccc91c7", kind: "immutable", lane: 1, rejoins: true, refs: [{ label: "main", kind: "bookmark" }] },
        { changeId: "ovvvnnor", desc: "Add a licence note", commitId: "ab228dc9", kind: "immutable" },
      ],
    },
    {
      command: "jj rebase -d main",
      caption: "Moved onto the new main: the change ID stays, the commit ID is new",
      rows: [
        { changeId: "upwouywx", desc: "Add contributing guide", commitId: "56f5616c", kind: "at" },
        { changeId: "zumzptvl", desc: "Add the copyright line", commitId: "eccc91c7", kind: "immutable", refs: [{ label: "main", kind: "bookmark" }] },
        { changeId: "ovvvnnor", desc: "Add a licence note", commitId: "ab228dc9", kind: "immutable" },
      ],
    },
  ]}
  ...
  status="{n}/{total} · {command} — {caption}"
  caption="A fetch does not move your work. The rebase is what moves it"
/>
