...
/**
 * The index list itself: highlights the heading the reader is at, keeps that
 * entry inside its own scroll window, and shows its scrollbar only while
 * moving. The desktop panel and the header's index button both render one.
 */
export function TocList({
  ...
  /** Called after an entry is chosen — the mobile panel closes on it. */
  ...
  // The list scrolls on its own once it is taller than its panel allows, so
  // the highlighted entry has to be brought into that scroller as the reader
  // moves through the post — otherwise it drifts out of sight and the index
  // stops saying where they are.
  useEffect(() => {
  ...
  return (
    <ul
      ...
      className={`pp-toc-list min-h-0 space-y-1 overflow-y-auto border-l border-cyber-line text-sm ${className}`}
    >
      {items.map((item) => {
